...
while isempty(ferror(fid)) && contador<maxlines %mientras no da fin de archivo
    ...
    for i=1:length(texts)% para cada uno de los textos de busqueda
        % take first col of tline
        tline_cols = strsplit(tline);
        col = tline_cols{1};
        if ~isempty(strfind(col,texts{i})) %me fijo si aparece en la linea
%            disp([tline ])
            linea=contador; 
            fclose(fid);
...
[start_line start_tline]=busca_texto_en_archivo(archivo, {'START'},1000);
if isempty(start_line)
    disp('Reached EOF and didn''t found any of corresponding texts')
    disp('Startline: 200')
    S=200;
else
    start_tline_cols = strsplit(start_tline); 
    [line tline]=busca_texto_en_archivo(archivo, start_tline_cols(2),1000);
    S=line-1;
    disp(['Startline: ' num2str(S+1)])
end
end