...
total_length  = 210*7+2; %this should always be Ntrials x 7 + 2
...
trial.vs_filenames = {txt{3:7:total_length,rows}};
% T_filenames
lookup = 'st5';
rows = find(cellfun(@(c) ischar(c) && strcmp(c, lookup), headers));
trial.T_filenames  = {txt{3:7:total_length,rows}};
% D1
lookup = 'st1';
rows = find(cellfun(@(c) ischar(c) && strcmp(c, lookup), headers));
trial.D1_filenames  = {txt{3:7:total_length,rows}};
% D2
lookup = 'st2';
rows = find(cellfun(@(c) ischar(c) && strcmp(c, lookup), headers));
trial.D2_filenames  = {txt{3:7:total_length,rows}};
% D3
lookup = 'st3';
rows = find(cellfun(@(c) ischar(c) && strcmp(c, lookup), headers));
trial.D3_filenames  = {txt{3:7:total_length,rows}};
% D4
lookup = 'st4';
rows = find(cellfun(@(c) ischar(c) && strcmp(c, lookup), headers));
trial.D4_filenames  = {txt{3:7:total_length,rows}};
% Stim present (cell 5 x Ntotal)
lookup = 'st1_cat';
rows = find(cellfun(@(c) ischar(c) && strcmp(c, lookup), headers));
stim_present{1}        =  cell2mat(txt(3:7:total_length,rows))=='D'; % Changed to 3 because empty row 2. Check why before it worked before with filenames
lookup = 'st2_cat';
rows = find(cellfun(@(c) ischar(c) && strcmp(c, lookup), headers));
stim_present{2}        =  cell2mat(txt(3:7:total_length,rows))=='D';
lookup = 'st3_cat';
rows = find(cellfun(@(c) ischar(c) && strcmp(c, lookup), headers));
stim_present{3}        =  cell2mat(txt(3:7:total_length,rows))=='D';
lookup = 'st4_cat';
rows = find(cellfun(@(c) ischar(c) && strcmp(c, lookup), headers));
stim_present{4}        =  cell2mat(txt(3:7:total_length,rows))=='D';
lookup = 'st5_cat';
rows = find(cellfun(@(c) ischar(c) && strcmp(c, lookup), headers));
stim_present{5}        =  cell2mat(txt(3:7:total_length,rows))=='T';
...
lookup = 'Nstim';
rows = find(cellfun(@(c) ischar(c) && strcmp(c, lookup), headers));
trial.Nstim        = num(7:7:total_length,rows);

% stimulus duration
lookup = 'stDur';
rows = find(cellfun(@(c) ischar(c) && strcmp(c, lookup), headers));
trial.stDurMem     = num(7:7:total_length,rows); %this stim duration depends on the number of stimuli
trial.stDurVS      = 10; %VS time search can last up to 10 secs

% responses
lookup = 'key_resp.corr';
rows = find(cellfun(@(c) ischar(c) && strcmp(c, lookup), headers));
trial.respcorr     = num(7:7:total_length,rows);

lookup = 'key_resp.rt';
rows = find(cellfun(@(c) ischar(c) && strcmp(c, lookup), headers));
trial.rt           = num(7:7:total_length,rows);
